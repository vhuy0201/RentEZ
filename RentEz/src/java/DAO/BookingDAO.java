...
public class BookingDAO {
    ...
    public boolean addProperty(Booking booking) {
        Connection conn = DBConnection.getConnection();
        String sql = "INSERT INTO Booking (RenterID, PropertyID, StartDate, EndDate, TotalPrice, Status, "
                    + "DepositAmount, MonthlyRent, PenaltyClause, TermsAndConditions, CreatedAt, SignedAt, "
                    + "SignedByRenter, SignedByLandlord) "
        ...
        try {
            ...
            pstmt.setDouble(6, booking.getDepositAmount());
            pstmt.setDouble(7, booking.getMonthlyRent());
            pstmt.setString(8, booking.getPenaltyClause());
            pstmt.setString(9, booking.getTermsAndConditions());

            // Handle SignedAt (it can be null)
            if (booking.getSignedAt() != null) {
                pstmt.setTimestamp(10, new java.sql.Timestamp(booking.getSignedAt().getTime()));
            } else {
                pstmt.setNull(10, java.sql.Types.TIMESTAMP);
            }

            pstmt.setBoolean(11, booking.isSignedByRenter());
            pstmt.setBoolean(12, booking.isSignedByLandlord());
    ...
    /**
     * Approve a contract by setting status to "Confirmed" and signedByLandlord to true
     * @param bookingId The ID of the booking to approve
     * @return true if successful, false otherwise
     */
    public boolean approveContract(int bookingId) {
        Connection conn = DBConnection.getConnection();
        String sql = "UPDATE Booking SET Status = 'Confirmed', SignedByLandlord = 1, SignedAt = GETDATE() WHERE BookingID = ?";
        try {
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, bookingId);
            int rows = pstmt.executeUpdate();
            ...
            System.out.println("Error approving contract: " + e);
        }
        return false;
    }

    /**
     * Reject a contract by setting status to "Rejected"
    ...
    public boolean rejectContract(int bookingId, String reason) {
        ...
        try {
            ...
            System.out.println("Error rejecting contract: " + e);
        }
        return false;
    }

    /**
     * Get all bookings for a specific landlord (based on property ownership)
     * @param landlordId The ID of the landlord
     * @return List of bookings for the landlord's properties
     */
    public List<Booking> getBookingsByLandlord(int landlordId) {
        List<Booking> bookings = new ArrayList<>();
        Connection conn = DBConnection.getConnection();
        String sql = "SELECT b.* FROM Booking b " +
                    "INNER JOIN Property p ON b.PropertyID = p.PropertyID " +
        ...
        try {
            ...
            System.out.println("Error getting bookings by landlord: " + e);
        }
        return bookings;
    }

    /**
     * Get contract statistics for a landlord
     * @param landlordId The ID of the landlord
     * @return Map containing counts for each status
     */
    public java.util.Map<String, Integer> getContractStats(int landlordId) {
        java.util.Map<String, Integer> stats = new java.util.HashMap<>();
        Connection conn = DBConnection.getConnection();
        String sql = "SELECT b.Status, COUNT(*) as Count FROM Booking b " +
                    "INNER JOIN Property p ON b.PropertyID = p.PropertyID " +
                    "WHERE p.LandlordID = ? " +
                    "GROUP BY b.Status";
        try {
            ...
            System.out.println("Error getting contract stats: " + e);
        }
        return stats;
    }
}
